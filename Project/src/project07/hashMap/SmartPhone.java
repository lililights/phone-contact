...
public class SmartPhone {
	...
	public Contact insert(int contactType, String thisPhoneNum) {
		String name = checkName();
		String phoneNum = checkPhoneNum();

		if (map.get(phoneNum) != null && !(phoneNum.equals(thisPhoneNum))) {
			System.out.println("--------------------------------");
			...
			if (contactType == 1) {
				map.put(phoneNum, new Contact(name, phoneNum, email, address, birthday, group));
	...
	public void printAll() {
		...
		while (itr.hasNext()) {
			String phoneNum = itr.next();
			map.get(phoneNum).print();
		}
		System.out.println("================================");
		System.out.print("메뉴로 돌아가려면 엔터를 누르세요.");
		scanner.nextLine();
	...
	public void update() {
		String phoneNum = checkStr("수정할 연락처의 전화번호를 입력하세요.\n> ");
		Contact c = map.get(phoneNum);

		if (c == null) {
			System.out.println("--------------------------------");
			System.out.println("해당 번호의 연락처 정보가 없습니다.");
			System.out.println("--------------------------------");
		} else {
			System.out.println("--------------------------------");
			System.out.println(c.getName() + "님의 연락처를 수정합니다.");
			System.out.println("--------------------------------");
			...
			Contact temp = insert(contactType, phoneNum);
			if (temp != null) {
				map.remove(c.getPhoneNum());
				map.put(temp.getPhoneNum(), temp);
				System.out.println("--------------------------------");
				System.out.println(c.getName() + "님의 연락처 정보가 변경되었습니다.");
				System.out.println("--------------------------------");
			}
		}
		System.out.print("메뉴로 돌아가려면 엔터를 누르세요.");
	...
	public void delete() {
		String phoneNum = checkStr("삭제할 연락처의 전화번호를 입력하세요.\n> ");
		Contact c = map.get(phoneNum);
		...
		if (c == null) {
			...
			map.remove(phoneNum);
			System.out.println("--------------------------------");
			System.out.println(c.getName() + "님의 연락처가 삭제 되었습니다.");
			System.out.println("--------------------------------");
		}
		System.out.print("메뉴로 돌아가려면 엔터를 누르세요.");
		scanner.nextLine();
	...
	public void search() {
		String phoneNum = checkStr("검색할 연락처의 전화번호를 입력하세요.\n> ");
		Contact c = map.get(phoneNum);
